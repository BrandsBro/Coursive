      ...
      <div style={{ textAlign:"center", maxWidth:480 }}>
        ...
        <div style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:24, padding:"40px 32px", marginBottom:24 }}>
          ...
          <p style={{ fontSize:16, color:"rgba(255,255,255,0.7)", margin:"0 0 24px", lineHeight:1.7 }}>
            Congratulations! 🎊 Your Coursiv account has been created and your login details have been sent to your email.
          </p>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <Link href="/login" style={{ display:"block", padding:"16px", borderRadius:14, border:"none", background:"linear-gradient(135deg,#5B4EFF,#8B5CF6)", color:"#fff", fontSize:16, fontWeight:700, textDecoration:"none", boxShadow:"0 8px 24px rgba(91,78,255,0.4)" }}>
              Log In to Coursiv →
            </Link>
          </div>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:32 }}>
          {[
            { icon:"🎓", label:"AI Certificate" },
            { icon:"📚", label:"All Courses" },
            { icon:"🏆", label:"Challenges" },
          ].map((item,i) => (
            <div key={i} style={{ textAlign:"center" }}>
              <div style={{ fontSize:28, marginBottom:4 }}>{item.icon}</div>
              <p style={{ fontSize:12, color:"rgba(255,255,255,0.5)", margin:0, fontWeight:600 }}>{item.label}</p>
            </div>
          ))}
        </div>
      </div>
