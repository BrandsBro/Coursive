...
export async function POST(req) {
  try {
    const { email, name, expiresAt } = await req.json();
    ...
    if (tmpl?.blocks?.length > 0) {
      ...
      html = `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a081e;color:#fff;border-radius:16px;padding:32px"><h2 style="color:#5B4EFF">Subscription Cancelled</h2><p style="color:rgba(255,255,255,0.7)">Hi ${name},</p><p style="color:rgba(255,255,255,0.7)">Your subscription has been cancelled. You will keep access until ${new Date(expiresAt).toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"})}.</p><p style="color:rgba(255,255,255,0.5)">The 1Course Team</p></div>`;
    }
