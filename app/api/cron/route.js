...
export async function GET(req) {
  ...
  try {
    ...
    for (const sub of expiring3 || []) {
      ...
      const renewalDate = new Date(sub.expires_at).toLocaleDateString("en-US", {
        month: "long",
        day: "numeric",
        year: "numeric",
      });
      const ok = await sendEmail(
        email,
        tmpl3.subject || "Subscription renews in 3 days",
        "<p>Hi " +
          name +
    ...
    for (const sub of expiring1 || []) {
      ...
      const ok = await sendEmail(
        email,
        tmpl1.subject || "Subscription renews tomorrow",
        "<p>Hi " +
          name +
          ", your subscription renews tomorrow on " +
          renewalDate +
          "</p>",
      );
      if (ok) results.sent++;
