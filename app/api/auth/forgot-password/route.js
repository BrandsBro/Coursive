...
function generateEmailHtml(blocks, resetLink, email, tmpl) {
...
export async function POST(req) {
  try {
    ...
    await supabase.from("password_resets").insert({
      user_id: profile.id, email, token, expires_at: expiresAt.toISOString(),
    });
    ...
    if (tmpl?.blocks?.length > 0) {
      ...
      html = generateEmailHtml(tmpl.blocks, resetLink, email, tmpl);
    } else {
