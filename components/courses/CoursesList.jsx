...
import { Search, BookOpen, Clock, Trophy, Play, CheckCircle2 } from "lucide-react";
          ...
          <div>
            ...
            <div className="cl-grid">
              {courses.filter(c => {
                ...
                const pct = getCoursePercent(c.id, total);
                return pct > 0 && pct < 100;
              }).map(course => <MiniCard key={course.id} course={course} getCoursePercent={getCoursePercent} isHighlighted />)}
            </div>
          </div>
        )}
...
function MiniCard({ course, getCoursePercent, isHighlighted }) {
  const s = S[course.id] || { g:"linear-gradient(135deg,#6366f1,#8b5cf6)", e:"📚", a:"#6366f1", al:"rgba(99,102,241,0.12)" };
    ...
    <Link href={`/courses/${course.id}`} style={{ textDecoration:"none", display:"block", height:"100%" }}>
      <div
        style={{
          ...
          border:`1.5px solid ${isHighlighted || isStarted ? s.a + "40" : "#F1F5F9"}`,
          boxShadow: isHighlighted || isStarted ? `0 4px 20px ${s.a}18` : "0 2px 8px rgba(0,0,0,0.05)",
          transition:"all 0.2s", height:"100%", display:"flex", flexDirection:"column",
        }}
        onMouseEnter={e => {
          e.currentTarget.style.transform = "translateY(-5px)";
          e.currentTarget.style.boxShadow = `0 16px 36px ${s.a}28`;
          e.currentTarget.style.borderColor = `${s.a}55`;
        }}
        onMouseLeave={e => {
          e.currentTarget.style.transform = "translateY(0)";
          e.currentTarget.style.boxShadow = isHighlighted || isStarted ? `0 4px 20px ${s.a}18` : "0 2px 8px rgba(0,0,0,0.05)";
          e.currentTarget.style.borderColor = isHighlighted || isStarted ? `${s.a}40` : "#F1F5F9";
        }}
      >
        {/* Hero */}
        <div style={{
          ...
          padding:"14px 16px", position:"relative", overflow:"hidden", flexShrink:0,
        }}>
          <div style={{ position:"absolute", top:-24, right:-24, width:90, height:90, borderRadius:"50%", background:"rgba(255,255,255,0.09)" }} />
          <div style={{ position:"absolute", bottom:-16, left:30, width:60, height:60, borderRadius:"50%", background:"rgba(255,255,255,0.06)" }} />
          {!course.imageUrl && (
            <span style={{ fontSize:40, position:"relative", zIndex:1, filter:"drop-shadow(0 3px 8px rgba(0,0,0,0.2))" }}>
              {s.e}
            </span>
          )}
          <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"flex-end", gap:5, marginLeft:"auto" }}>
            ...
            {isDone && (
              <div style={{ background:"rgba(34,197,94,0.9)", borderRadius:999, padding:"2px 8px", display:"flex", alignItems:"center", gap:3 }}>
                <CheckCircle2 size={9} color="#fff" />
                <span style={{ color:"#fff", fontSize:9, fontWeight:700 }}>Complete</span>
              </div>
            )}
            {isStarted && !isDone && (
              <div style={{ background:"rgba(255,255,255,0.18)", borderRadius:999, padding:"2px 8px" }}>
                <span style={{ color:"#fff", fontSize:9, fontWeight:700 }}>{pct}%</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ padding:"13px 14px 14px", flex:1, display:"flex", flexDirection:"column" }}>
          <h3 style={{ fontSize:13, fontWeight:800, color:"#0f172a", margin:"0 0 3px", lineHeight:1.3 }}>
            {course.title}
          </h3>
          <p style={{
            fontSize:11, color:"#64748B", margin:"0 0 10px", lineHeight:1.5,
          ...
          <div style={{ display:"flex", gap:4, marginBottom:10, flexWrap:"wrap" }}>
            {[
              { icon:<BookOpen size={9} color={s.a} />, text:`${total} lessons` },
              { icon:<Clock    size={9} color={s.a} />, text:`${course.hours}h` },
              { icon:<Trophy  size={9} color={s.a} />, text:course.category },
            ].map((m, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:3, background:s.al, borderRadius:6, padding:"3px 7px" }}>
                {m.icon}
                <span style={{ fontSize:10, fontWeight:600, color:s.a }}>{m.text}</span>
              </div>
            ))}
          </div>
            ...
            {isStarted ? (
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontSize:10, color:"#94A3B8" }}>{isDone ? "Completed!" : "In progress"}</span>
                  <span style={{ fontSize:10, fontWeight:700, color:s.a }}>{pct}%</span>
                </div>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:s.al, borderRadius:10, padding:"8px 11px" }}>
                <span style={{ fontSize:12, fontWeight:700, color:s.a }}>Start course</span>
                <div style={{ width:22, height:22, borderRadius:"50%", background:s.g, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
