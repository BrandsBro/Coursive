    ...
    case "blankoptions": {
      ...
      return (
        <div style={{ padding:"8px 0 24px" }}>
          {/* Sentence */}
          <p style={{ fontSize:18, color:"#0f172a", margin:"0 0 28px", lineHeight:2.6, fontWeight:500 }}>
            {sentenceParts.map((part,i) => {
              ...
              const checked2 = isChecked;
              const ok = sel===correct;
              return (
                <span key={i} style={{ display:"inline-block", minWidth:80, padding:"4px 16px", margin:"0 4px", borderRadius:10, border:`2px solid ${checked2?(ok?"#22c55e":"#ef4444"):sel?"#111827":"#D1D5DB"}`, background:"#fff", color:checked2?(ok?"#166534":"#991B1B"):sel?"#111827":"#9CA3AF", fontWeight:700, fontSize:16, textAlign:"center", verticalAlign:"middle", transition:"all 0.2s", boxShadow:checked2?"none":sel?"0 2px 0 #111827":"0 2px 0 #D1D5DB" }}>
                  {sel||"_____"}
                </span>
              );
            })}
                    ...
                    {getOptions(i).map((opt,j) => {
                      ...
                      return (
                        <button key={j} onClick={() => setAnswers(p=>({...p,["bo_"+idx]:{...selectedMap,[String(i)]:sel===opt?undefined:opt}}))}
                          style={{ padding:"10px 22px", borderRadius:12, border:`2px solid ${isSelected?"#111827":"#E5E7EB"}`, background:"#fff", color:"#111827", fontSize:15, fontWeight:600, cursor:"pointer", boxShadow:isSelected?"0 2px 0 #111827":"0 2px 0 #D1D5DB", transition:"all 0.1s" }}>
                          {opt}
                        </button>
                      );
                    })}
          ...
          {isChecked && (
            <div>
              {allCorrect ? (
                <div>
                  <div style={{ padding:"16px 18px", borderRadius:14, background:"#F0FDF4", border:"1.5px solid #BBF7D0", marginBottom:16 }}>
                  ...
                  {(c.successImages||[]).filter(Boolean).map((url,i) => <img key={i} src={url} alt="" style={{ width:"100%", borderRadius:14, display:"block", marginBottom:10 }}/>)}
                </div>
              ) : (
                <div>
                  <div style={{ padding:"14px 18px", borderRadius:14, background:"#FEF2F2", border:"1.5px solid #FECACA", marginBottom:12 }}>
