  ...
  const formContent = (
    <div style={{ width:"100%" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {!isLogin && (
          <InputField icon={<User size={14} color="#818cf8"/>} placeholder="Full name" value={name} onChange={setName} onEnter={handleSubmit}/>
        )}
      ...
      <button onClick={handleSubmit} disabled={loading}
        ...
        {loading ? (
          <><div style={{ width:15, height:15, borderRadius:"50%", border:"2px solid rgba(255,255,255,0.3)", borderTopColor:"#fff", animation:"spin 0.7s linear infinite" }}/>{isLogin ? "Signing in..." : "Creating account..."}</>
        ) : (
      ...
      {!isLogin && (
        <p style={{ textAlign:"center", color:"rgba(255,255,255,0.2)", fontSize:11, margin:"10px 0 0" }}>
          <Link href="/quiz" style={{ color:"rgba(255,255,255,0.3)", textDecoration:"none" }}>← Take the quiz first</Link>
        </p>
      )}
