...
function EndBlock({ step, loadingPct, email, setEmail, name, setName, answers, blocks, showToast }) {
  ...
  if (step === "sales") {
    const plans = [
      { name:"1-Week Plan", price:"6.93", weeks:1 },
      { name:"4-Week Plan", price:"19.99", weeks:4, popular:true },
      { name:"12-Week Plan", price:"39.99", weeks:12 },
    ];
    return (
      <>
        ...
        {showPayment && (
          <PaymentModal plan={selectedPlan} email={email} name={name} onClose={() => setShowPayment(false)} onSuccess={handlePaymentSuccess}/>
        )}
      </>
    );
  }
