...
import { Save, Check, Loader, Send, ArrowLeft, Plus, Trash2, ChevronUp, ChevronDown, Copy } from "lucide-react";
...
function generateHtml(blocks, name, email, password, template) {
  ...
  const blockHtml = (blocks||[]).map(b => {
    switch(b.type) {
      case "header":
        return `<div style="background:linear-gradient(135deg,${b.bg1||"#5B4EFF"},${b.bg2||"#8B5CF6"});padding:${b.padding||48}px 40px;text-align:center"></div>`;
      case "logo":
        if (!b.logoUrl) return `<div style="padding:20px 40px;text-align:${b.align||"center"}"><p style="color:rgba(255,255,255,0.3);font-size:12px;margin:0">[Logo placeholder]</p></div>`;
        return `<div style="padding:20px 40px;text-align:${b.align||"center"}"><img src="${b.logoUrl}" alt="Logo" style="height:${b.logoHeight||44}px;object-fit:contain;display:inline-block"/></div>`;
      case "heading":
      ...
      case "image":
        if (!b.url) return "";
        return `<div style="padding:8px 40px;text-align:${b.align||"center"}"><img src="${b.url}" alt="" style="width:100%;border-radius:${b.radius||12}px;display:block"/></div>`;
      case "divider":
        return `<div style="margin:${b.margin||24}px 40px;border-top:1px solid ${b.color||"rgba(255,255,255,0.08)"}"></div>`;
      case "spacer":
      ...
      case "footer":
        return `<div style="padding:${b.padding||24}px 40px;border-top:1px solid rgba(255,255,255,0.06);text-align:${b.align||"center"};background:${b.bg||"transparent"}"><p style="color:${b.color||"rgba(255,255,255,0.3)"};font-size:${b.size||12}px;margin:0;line-height:1.6">${replace(b.text)}</p></div>`;
      default: return "";
    }
  }).join("");
...
export default function EmailTemplateEditor({ templateId }) {
  ...
  const updateBlock = (id, key, val) => {
    setTemplate(prev => ({ ...prev, blocks: prev.blocks.map(b => b.id===id ? {...b,[key]:val} : b) }));
  };

  const addBlock = (type) => {
    const nb = { id:"b"+Date.now(), type, ...getBlockDefaults(type) };
    setTemplate(prev => ({ ...prev, blocks:[...(prev.blocks||[]), nb] }));
    setSelectedId(nb.id); setShowAdd(false);
  };
  ...
  const moveBlock = (id, dir) => {
    ...
    setTemplate(prev => ({ ...prev, blocks:arr }));
  };

  const sendTest = async () => {
    if (!testEmail) return;
    setSending(true);
    const html = generateHtml(blocks, "John Smith", testEmail, "testPass123!", template);
    await fetch("/api/send-email", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ to:testEmail, subject:template.subject||"Test Email", html }) });
    setSentOk(true); setTimeout(()=>setSentOk(false),3000); setSending(false);
  };

  const uploadImg = async (file, blockId) => {
    if (!file) return;
    setUploading(true);
    const path = `email/${Date.now()}-${file.name}`;
    const { error } = await supabase.storage.from("lesson-media").upload(path, file, { upsert:true });
    if (!error) { const { data } = supabase.storage.from("lesson-media").getPublicUrl(path); updateBlock(blockId, "logoUrl", data.publicUrl); }
    setUploading(false);
  };
                ...
                <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:3 }}>
                  {BLOCK_TYPES.map(bt => (
                    <button key={bt.type} onClick={()=>addBlock(bt.type)}
                      style={{ padding:"7px 10px", borderRadius:7, border:"1.5px solid #E2E8F0", background:"#fff", fontSize:12, fontWeight:600, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:8 }}>
                ...
                return (
                  <div key={b.id}
                    ...
                    onDrop={()=>{
                      ...
                      const ti=arr.findIndex(x=>x.id===b.id);
...
function BlockControls({ block:b, onChange, uploadImg, uploading }) {
  switch(b.type) {
    case "header": return (
      <div style={col()}>
        <CF label="Gradient Start" value={b.bg1||"#5B4EFF"} onChange={v=>onChange("bg1",v)}/>
        <CF label="Gradient End" value={b.bg2||"#8B5CF6"} onChange={v=>onChange("bg2",v)}/>
        <SF label="Padding (px)" value={b.padding||48} onChange={v=>onChange("padding",v)}/>
      </div>
    );
    case "logo": return (
      <div style={col()}>
        ...
        <label style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"9px", borderRadius:9, border:"1.5px dashed #C7D2FE", background:"#EEF2FF", color:"#5B4EFF", fontSize:12, fontWeight:700, cursor:"pointer" }}>
          {uploading?"⏳ Uploading...":"⬆ Upload Logo"}
          <input type="file" accept="image/*" style={{ display:"none" }} onChange={e=>uploadImg(e.target.files[0],b.id)}/>
        </label>
        {b.logoUrl && <img src={b.logoUrl} alt="" style={{ maxHeight:40, objectFit:"contain", borderRadius:6 }}/>}
        <SF label="Height (px)" value={b.logoHeight||44} onChange={v=>onChange("logoHeight",v)}/>
        <AF label="Align" value={b.align||"center"} onChange={v=>onChange("align",v)}/>
      </div>
    );
    case "heading": return (
      <div style={col()}>
        <div><p style={lbl()}>Text <span style={{ color:"#94A3B8", fontWeight:400 }}>· {"{name}"} = user name</span></p><textarea value={b.text||""} onChange={e=>onChange("text",e.target.value)} rows={3} style={{ ...inp(), resize:"vertical" }}/></div>
        <CF label="Color" value={b.color||"#fff"} onChange={v=>onChange("color",v)}/>
        <SF label="Font Size" value={b.size||22} onChange={v=>onChange("size",v)}/>
        <AF label="Align" value={b.align||"left"} onChange={v=>onChange("align",v)}/>
        <div style={{ display:"flex", gap:6 }}>
          <button onClick={()=>onChange("bold",!b.bold)} style={{ flex:1, padding:"6px", borderRadius:7, border:`1.5px solid ${b.bold?"#5B4EFF":"#E2E8F0"}`, background:b.bold?"#EEF2FF":"#fff", fontWeight:900, fontSize:14, cursor:"pointer" }}>B</button>
          <button onClick={()=>onChange("italic",!b.italic)} style={{ flex:1, padding:"6px", borderRadius:7, border:`1.5px solid ${b.italic?"#5B4EFF":"#E2E8F0"}`, background:b.italic?"#EEF2FF":"#fff", fontStyle:"italic", fontWeight:700, fontSize:14, cursor:"pointer" }}>I</button>
        </div>
      </div>
    );
    case "text": return (
    ...
    case "button": return (
      <div style={col()}>
        ...
        <CF label="BG Start" value={b.bgFrom||"#5B4EFF"} onChange={v=>onChange("bgFrom",v)}/>
        <CF label="BG End" value={b.bgTo||"#8B5CF6"} onChange={v=>onChange("bgTo",v)}/>
        <CF label="Text Color" value={b.color||"#fff"} onChange={v=>onChange("color",v)}/>
        <SF label="Font Size" value={b.size||16} onChange={v=>onChange("size",v)}/>
        <SF label="Border Radius" value={b.radius||14} onChange={v=>onChange("radius",v)}/>
        <SF label="Padding Vertical" value={b.paddingV||16} onChange={v=>onChange("paddingV",v)}/>
        <SF label="Padding Horizontal" value={b.paddingH||32} onChange={v=>onChange("paddingH",v)}/>
        <AF label="Align" value={b.align||"center"} onChange={v=>onChange("align",v)}/>
      </div>
    );
    case "image": return (
      <div style={col()}>
        ...
        <label style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"9px", borderRadius:9, border:"1.5px dashed #C7D2FE", background:"#EEF2FF", color:"#5B4EFF", fontSize:12, fontWeight:700, cursor:"pointer" }}>
          ...
          <input type="file" accept="image/*" style={{ display:"none" }} onChange={async e=>{
            ...
            const path=`email/${Date.now()}-${file.name}`;
            const {error}=await supabase.storage.from("lesson-media").upload(path,file,{upsert:true});
            if(!error){const{data}=supabase.storage.from("lesson-media").getPublicUrl(path);onChange("url",data.publicUrl);}
          }}/>
        </label>
        {b.url && <img src={b.url} alt="" style={{ width:"100%", borderRadius:6, maxHeight:80, objectFit:"cover" }}/>}
        <SF label="Border Radius" value={b.radius||12} onChange={v=>onChange("radius",v)}/>
        <AF label="Align" value={b.align||"center"} onChange={v=>onChange("align",v)}/>
      </div>
    );
    case "divider": return (
      <div style={col()}>
        <CF label="Color" value={b.color||"rgba(255,255,255,0.08)"} onChange={v=>onChange("color",v)}/>
        <SF label="Margin (px)" value={b.margin||24} onChange={v=>onChange("margin",v)}/>
      </div>
    );
    case "spacer": return (
      <div style={col()}><SF label="Height (px)" value={b.height||24} onChange={v=>onChange("height",v)}/></div>
    );
    case "credentials": return (
      <div style={col()}>
        <CF label="Label Color" value={b.labelColor||"rgba(255,255,255,0.5)"} onChange={v=>onChange("labelColor",v)}/>
        <CF label="Email Color" value={b.emailColor||"#a78bfa"} onChange={v=>onChange("emailColor",v)}/>
        <CF label="Password Text" value={b.pwColor||"#fff"} onChange={v=>onChange("pwColor",v)}/>
        <CF label="Password BG" value={b.pwBg||"rgba(91,78,255,0.2)"} onChange={v=>onChange("pwBg",v)}/>
        <CF label="Card BG" value={b.cardBg||"#1a1830"} onChange={v=>onChange("cardBg",v)}/>
        <CF label="Card Border" value={b.cardBorder||"rgba(255,255,255,0.1)"} onChange={v=>onChange("cardBorder",v)}/>
      </div>
    );
    case "footer": return (
      <div style={col()}>
        <div><p style={lbl()}>Text</p><textarea value={b.text||""} onChange={e=>onChange("text",e.target.value)} rows={3} style={{ ...inp(), resize:"vertical" }}/></div>
        <CF label="Text Color" value={b.color||"rgba(255,255,255,0.3)"} onChange={v=>onChange("color",v)}/>
        <CF label="Background" value={b.bg||"transparent"} onChange={v=>onChange("bg",v)}/>
        <SF label="Font Size" value={b.size||12} onChange={v=>onChange("size",v)}/>
        <SF label="Padding" value={b.padding||24} onChange={v=>onChange("padding",v)}/>
        <AF label="Align" value={b.align||"center"} onChange={v=>onChange("align",v)}/>
      </div>
    );
    default: return null;
