  ...
  plans: [
    {
      ...
      perDayPrice: "0.99",
      perDayOriginalPrice: "1.98",
      legalText: "By clicking Get My Plan, I agree to pay $6.93 for my 1-Week Plan.",
    },
    {
      id: "monthly",
      name: "4-Week Plan",
      ...
      perDayPrice: "0.71",
      perDayOriginalPrice: "1.43",
      legalText: "By clicking Get My Plan, I agree to pay $19.99 for my 4-Week Plan.",
    },
    {
      id: "quarterly",
      name: "12-Week Plan",
      ...
      perDayPrice: "0.48",
      perDayOriginalPrice: "0.95",
      legalText: "By clicking Get My Plan, I agree to pay $39.99 for my 12-Week Plan.",
    },
  ],
};
...
export default function PricingManager() {
  ...
  const updatePlan = (id, key, val) =>
    setPricing(prev => ({ ...prev, plans: prev.plans.map(p => p.id === id ? { ...p, [key]: val } : p) }));
  const u = (key, val) => setPricing(prev => ({ ...prev, [key]: val }));

  if (loading) return (
    <AdminLayout>
      <div style={{ padding:60, textAlign:"center" }}><Loader size={28} color="#94A3B8"/></div>
    </AdminLayout>
  );
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ background:"#EEF2FF", borderRadius:14, border:"1.5px solid #C7D2FE", padding:"14px 18px" }}>
              <p style={{ fontSize:13, color:"#4338CA", margin:0, fontWeight:600 }}>
                💡 Sale price is what users pay. Original price is shown crossed out. Per day prices are display-only on the plan page.
              </p>
            </div>
            ...
            {pricing.plans.map(plan => (
              <div key={plan.id} style={{ background:"#fff", borderRadius:20, border:`1.5px solid ${plan.popular?"#C7D2FE":"#F1F5F9"}`, padding:24, position:"relative" }}>
                {plan.popular && (
                  <div style={{ position:"absolute", top:-12, left:24, background:"#5B4EFF", color:"#fff", fontSize:11, fontWeight:800, padding:"3px 14px", borderRadius:999 }}>
                    MOST POPULAR
                  </div>
                )}
                ...
                {/* Row 1: core pricing */}
                <p style={{ fontSize:11, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:0.5, margin:"0 0 8px" }}>Core Pricing</p>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:12, marginBottom:16 }}>
                  ...
                  <div>
                    <label style={lbl()}>Sale Price ({pricing.currencySymbol})</label>
                    <input value={plan.salePrice} onChange={e => updatePlan(plan.id, "salePrice", e.target.value)} style={{ ...inp(), fontWeight:700, color:"#5B4EFF" }}/>
                  </div>
                  <div>
                    <label style={lbl()}>Original Price ({pricing.currencySymbol})</label>
                    <input value={plan.originalPrice} onChange={e => updatePlan(plan.id, "originalPrice", e.target.value)} style={inp()}/>
                  </div>
                  <div>
                    <label style={lbl()}>Discount Badge %</label>
                    <input value={plan.discountPct} onChange={e => updatePlan(plan.id, "discountPct", e.target.value)} style={inp()}/>
                  </div>
                </div>

                {/* Row 2: per-day + description */}
                <p style={{ fontSize:11, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:0.5, margin:"0 0 8px" }}>
                  Per Day Pricing <span style={{ color:"#C7D2FE", fontWeight:500, textTransform:"none" }}>(display only — no effect on payment)</span>
                </p>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 2fr", gap:12, marginBottom:16 }}>
                  <div>
                    <label style={lbl()}>Per Day Sale Price ({pricing.currencySymbol})</label>
                    <input
                      value={plan.perDayPrice||""}
                      placeholder="e.g. 0.99"
                      onChange={e => updatePlan(plan.id, "perDayPrice", e.target.value)}
                      style={{ ...inp(), fontWeight:700, color:"#7C3AED" }}
                    />
                  </div>
                  <div>
                    <label style={lbl()}>Per Day Original ({pricing.currencySymbol})</label>
                    <input
                      value={plan.perDayOriginalPrice||""}
                      placeholder="e.g. 1.98"
                      onChange={e => updatePlan(plan.id, "perDayOriginalPrice", e.target.value)}
                      style={inp()}
                    />
                  </div>
                  <div>
                ...
                {/* Price preview */}
                <div style={{ padding:"14px 18px", borderRadius:12, background:"#F8FAFC", border:"1.5px solid #E2E8F0", display:"flex", gap:32, alignItems:"center", flexWrap:"wrap" }}>
                  {/* Total price preview */}
                  <div>
                    <p style={{ fontSize:11, color:"#94A3B8", margin:"0 0 4px", fontWeight:600 }}>Total shown to user</p>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ fontSize:12, color:"#94A3B8", textDecoration:"line-through" }}>{pricing.currencySymbol}{plan.originalPrice}</span>
                  ...
                  {/* Per day preview */}
                  <div>
                    <p style={{ fontSize:11, color:"#94A3B8", margin:"0 0 4px", fontWeight:600 }}>Per day shown to user</p>
                    <div style={{ display:"flex", alignItems:"flex-start", gap:2 }}>
                      <span style={{ fontSize:13, fontWeight:800, color:"#7C3AED", marginTop:2 }}>{pricing.currencySymbol}</span>
                      <span style={{ fontSize:26, fontWeight:900, color:"#7C3AED", lineHeight:1 }}>
                        {(plan.perDayPrice||"0").split(".")[0]}
                      </span>
                      <div style={{ display:"flex", flexDirection:"column", marginTop:2 }}>
                        <span style={{ fontSize:13, fontWeight:800, color:"#7C3AED", lineHeight:1 }}>
                          {(plan.perDayPrice||"").includes(".") ? "." + plan.perDayPrice.split(".")[1] : ""}
                        </span>
                        <span style={{ fontSize:10, color:"#94A3B8", fontWeight:600 }}>Per day</span>
                      </div>
                      {plan.perDayOriginalPrice && (
                        <span style={{ fontSize:11, color:"#94A3B8", textDecoration:"line-through", marginTop:"auto", marginLeft:6 }}>
                          {pricing.currencySymbol}{plan.perDayOriginalPrice}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={{ background:"#FFFBEB", borderRadius:14, border:"1.5px solid #FDE68A", padding:"14px 18px" }}>
              <p style={{ fontSize:13, color:"#92400e", margin:0, fontWeight:600 }}>
                ⚠️ Each plan has its own legal text shown below the payment button. Be clear and accurate — this is a legal disclosure.
              </p>
            </div>
            {pricing.plans.map(plan => (
        ...
        {activeTab === "preview" && (
          <div style={{ background:"#fff", borderRadius:20, border:"1.5px solid #F1F5F9", padding:24 }}>
            ...
            <div style={{ maxWidth:480, margin:"0 auto", background:`linear-gradient(135deg,${pricing.bgGradientFrom},${pricing.bgGradientTo})`, borderRadius:20, padding:24 }}>
              <h2 style={{ fontSize:20, fontWeight:900, color:pricing.headingColor, textAlign:"center", margin:"0 0 20px" }}>
                Your A.I. Program is Ready!
              </h2>
              <div style={{ display:"flex", flexDirection:"column", gap:0, marginBottom:16 }}>
                {pricing.plans.filter(p=>p.active).map((plan, idx) => (
                  <div key={plan.id}>
                    {plan.popular && (
                      <div style={{ background:pricing.popularBadgeBg, color:pricing.popularBadgeColor, fontSize:11, fontWeight:800, padding:"8px 16px", textAlign:"center", borderRadius:"10px 10px 0 0", marginTop:8 }}>
                        👍 MOST POPULAR
                      </div>
                    )}
                    <div style={{ padding:"14px 16px", borderRadius: plan.popular ? "0 0 10px 10px" : 10, border:`2px solid ${plan.popular ? pricing.cardSelectedBorder : pricing.cardBorder}`, background: plan.popular ? pricing.cardSelectedBg : pricing.cardBg, marginBottom:8, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ width:16, height:16, borderRadius:"50%", border:`2px solid ${plan.popular?"#7C3AED":"#CBD5E1"}`, background: plan.popular?"#7C3AED":"transparent", flexShrink:0 }}/>
                        <div>
                          <div style={{ fontSize:13, fontWeight:800, color:pricing.headingColor }}>{plan.name.toUpperCase().replace(" ","-")}</div>
                          <div style={{ fontSize:11, color:pricing.descColor, marginTop:2 }}>
                            <span style={{ textDecoration:"line-through", marginRight:6 }}>{pricing.currencySymbol}{plan.originalPrice}</span>
                            <span style={{ fontWeight:700 }}>{pricing.currencySymbol}{plan.salePrice}</span>
                          </div>
                        </div>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end" }}>
                        <div style={{ display:"flex", alignItems:"flex-start", gap:1 }}>
                          <span style={{ fontSize:12, fontWeight:800, color:pricing.priceColor, marginTop:2 }}>{pricing.currencySymbol}</span>
                          <span style={{ fontSize:28, fontWeight:900, color:pricing.priceColor, lineHeight:1 }}>
                            {(plan.perDayPrice||"0").split(".")[0]}
                          </span>
                          <div style={{ display:"flex", flexDirection:"column", marginTop:2 }}>
                            <span style={{ fontSize:12, fontWeight:800, color:pricing.priceColor }}>
                              {(plan.perDayPrice||"").includes(".") ? "."+plan.perDayPrice.split(".")[1] : ""}
                            </span>
                            <span style={{ fontSize:9, color:pricing.descColor, fontWeight:600 }}>Per day</span>
                          </div>
                        </div>
                        {plan.perDayOriginalPrice && (
                          <span style={{ fontSize:10, color:pricing.descColor, textDecoration:"line-through" }}>
                            {pricing.currencySymbol}{plan.perDayOriginalPrice}
                          </span>
                        )}
                      </div>
