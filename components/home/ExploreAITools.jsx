        ...
        {courses.map((course) => {
          ...
          return (
            <Link key={course.id} href={`/courses/${course.id}`} style={{ textDecoration:"none" }}>
              ...
              >
                <div style={{ background:s.g,height:108,display:"flex",alignItems:"center",justifyContent:"center",fontSize:44,position:"relative",overflow:"hidden" }}>
                  <div style={{ position:"absolute",top:-20,right:-20,width:80,height:80,borderRadius:"50%",background:"rgba(255,255,255,0.1)" }} />
                  <span style={{ position:"relative",zIndex:1 }}>{s.e}</span>
                  {pct === 100 && (
                    <div style={{ position:"absolute",top:8,right:8,background:"#22C55E",color:"#fff",fontSize:8,fontWeight:700,padding:"2px 7px",borderRadius:999,letterSpacing:0.5 }}>✓ DONE</div>
                  )}
                </div>
