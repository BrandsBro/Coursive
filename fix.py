...
export async function POST(req) {
  try {
    ...
    if (existingProfile) {
      ...
      if (userError) { console.error("Create user error:", userError); throw userError; }
    ...
    await supabase.from("payment_history").insert({
      user_id: userId, subscription_id: sub?.id, amount, status: "succeeded",
      stripe_payment_id: paymentIntentId, plan, type: paymentType,
    });
    ...
    try {
      ...
      if (tmpl && tmpl.blocks && tmpl.blocks.length > 0) {
        ...
        emailHtml = generateEmailHtml(tmpl.blocks, name, email, tempPassword, tmpl);
      }
    } catch(e) { console.error("Template load error:", e); }

    if (!emailHtml) {
      emailHtml = fallbackEmailHtml(name, email, tempPassword, plan, paymentType, expiresAt, process.env.NEXT_PUBLIC_SITE_URL);
    }

    const { error: emailError } = await resend.emails.send({
      from: "noreply@1course.io", to: email, subject: emailSubject, html: emailHtml,
    });
...
open('app/api/stripe/create-account/route.js', 'w', encoding='utf-8').write(code)
print("Done!")