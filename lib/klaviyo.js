...
export async function addToKlaviyoList(listId, profile) {
  try {
    ...
    if (!profileId) {
      const profBody = await profileRes.text();
    console.error("Klaviyo profile error - status:", profileRes.status, "body:", profBody.slice(0,500));
      return false;
    }
    ...
    console.log("Klaviyo add to list status:", listRes.status, "body:", listResText.slice(0,500));
